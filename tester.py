...
def print_usage():
    print("Usage: python tester.py [search_type] [testcase_file] [-v (optional)] [--no-storm-particles (optional)]")
    ...
    print("    use --no-storm-particles to disable the storm particle effect in the visualiser")



def main(arglist):
    if len(arglist) not in {2, 3, 4}:
        print_usage()
        return
    ...
    for option in arglist[2:]:
        if option == '-v':
            visualise = True
        elif option == '--no-storm-particles':
    ...
    if actions is None:
        print(f"/!\\ Your {search_type} method did not return a path (it returned None).")
        print("    Implement it in solution.py and return a list of actions.")
        return
    ...
    for i in range(len(actions)):
        ...
        try:
            ...
            persistent_state, success, err_msg = game_env.perform_action(persistent_state, a)
            if not success:
